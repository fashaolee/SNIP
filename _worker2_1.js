...
// 并发回源的延迟阈值（毫秒）：直连优先，超过该时长仍无下行则并发代理
...
async function handleTCPOutBoundFast(remoteWrapper, address, port, initData, ws, vlessHeader) {
    ...
    function sendToWS(chunk) {
        ...
        if (headerPending) {
            ws.send(concatArrayBuffers(headerPending, chunk));
            headerPending = null;
        } else {
            ws.send(chunk);
        }
    ...
    transform.readable.pipeTo(new WritableStream({
        async write(dQuery) {
            const resp = await fetch('https://dns.google/dns-query', {
                ...
                body: dQuery, // 修正：原代码误写为 dnsQuery
            });
