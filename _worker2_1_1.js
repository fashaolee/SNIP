...
// 合包参数（可按需调大以拉满吞吐）
const WS_TO_REMOTE_COALESCE_BYTES = 32 * 1024;     // WS -> TCP
const REMOTE_TO_WS_COALESCE_BYTES = 64 * 1024;     // TCP -> WS
const COALESCE_TIME_MS = 4;                        // 两向通用最大延迟
...
// 简易写入合包器：按阈值/时间合并写入
function createWriteCoalescer(writer, bytesThreshold, timeMs) {
  let queue = [];
  let total = 0;
  let timer = null;
  let flushing = false;
  async function flush() {
    ...
    if (total === 0) return;
    flushing = true;
    const payload = concatArrayBuffers(...queue);
    queue = [];
    total = 0;
    try {
  ...
  return {
    async write(chunk) {
      ...
      if (total >= bytesThreshold) {
        if (timer) { try { clearTimeout(timer); } catch {} timer = null; }
        await flush(); // 大块触发，等待背压
      } else {
        schedule(); // 小块合包，低时延 flush
      }
    },
    async flushNow() {
      if (timer) { try { clearTimeout(timer); } catch {} timer = null; }
...
async function vlessOverWSHandler(request) {
  ...
  // 连接状态机：idle -> connecting -> ready
  let remote = { value: null, writer: null, state: 'idle', buffered: [], coalescer: null };
  let udpWrite = null;
  let isDns = false;
  ...
  readableWS.pipeTo(new WritableStream({
    async write(chunk) {
      ...
      // 2. TCP：若已 ready，直接走写入合包器
      if (remote.state === 'ready' && remote.coalescer) {
        await remote.coalescer.write(chunk);
        return;
      }

      // 3. TCP：若已 connecting，先缓冲以避免重复解析头
...
async function handleTCPOutBoundFast(remoteWrapper, address, port, initData, ws, vlessHeader) {
  ...
  function wsFlush() {
    if (wsBytes === 0) return;
    const payload = concatArrayBuffers(...wsBufs);
    wsBufs = [];
    wsBytes = 0;

    if (ws.readyState !== WS_READY_STATE_OPEN) return;
    if (headerPending) {
      ws.send(concatArrayBuffers(headerPending, payload));
      headerPending = null;
    } else {
      ws.send(payload);
    }
  }
  function wsPush(chunk) {
    wsBufs.push(chunk);
    wsBytes += chunk.byteLength || chunk.length || 0;
    if (wsBytes >= REMOTE_TO_WS_COALESCE_BYTES) {
      if (wsTimer) { try { clearTimeout(wsTimer); } catch {} wsTimer = null; }
      wsFlush();
    } else if (!wsTimer) {
      wsTimer = setTimeout(() => {
        wsTimer = null;
    ...
    try {
      while (true) {
        ...
        if (!selected) {
          ...
          // 选定写入方向：复用单一 writer + WS->远端合包器
          remoteWrapper.value = sock;
          remoteWrapper.writer = sock.writable.getWriter();
          remoteWrapper.coalescer = createWriteCoalescer(
            remoteWrapper.writer,
            WS_TO_REMOTE_COALESCE_BYTES,
            COALESCE_TIME_MS
          );

          // 冲刷 WS->远端 的已缓冲数据（保持顺序）
          const pending = remoteWrapper.buffered || [];
          if (pending.length) {
            for (const buf of pending) {
              await remoteWrapper.coalescer.write(buf);
            }
            pending.length = 0;
          }
          remoteWrapper.state = 'ready';
